...
browser.get('http://duan-imperiaskygarden.site/')
...
def ffos(name,emails,phones):
    username = browser.find_element_by_name('subscriber_name')
    email = browser.find_element_by_name('subscriber_email')
    phone = browser.find_element_by_name('subscriber_phone')
    username.send_keys(name)
    email.send_keys(emails)
    phone.send_keys(phones, Keys.RETURN)
    time.sleep(5)
